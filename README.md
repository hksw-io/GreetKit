...
- `.softGradient` — a restrained brand-derived background tuned for readable onboarding content.
...
The built-in presets are `.subtle`, `.standard`, and `.expressive`. Stronger motion increases movement, speed, and gradient contrast. For finer control, pass `OnboardingGradientMotion(strength:)`.
